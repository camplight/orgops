export function buildSystemPrompt(docsText: string) {
  ...
  const sections = [
    ...
    `  - pm2 start ${npmCommand} --name orgops-admin-ui --cwd "./orgops" -- run start:admin-ui:preview:env`,
    `  - pm2 start ${npmCommand} --name orgops-user-ui --cwd "./orgops" -- run start:user-ui:preview:env`,
    "  - pm2 save",
    "",
    "Response policy:",
