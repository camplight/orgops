...
function printCliHelp() {
  const lines = [
    ...
    "  --dir <path>         Install/admin/service: OrgOps install directory (default: ./orgops)",
...
async function main() {
  ...
  if (parsed.name === "service-register") {
    try {
      ...
      writeRoleMessage("opscli", `Services registered for install dir: ${result.installDir}`);
      writeRoleMessage("opscli", `Components: ${formatComponents(result.serviceComponents)}`);
      for (const message of result.messages) writeRoleMessage("opscli", message);
    } catch (error) {
  ...
  if (parsed.name === "service-unregister") {
    try {
      ...
      writeRoleMessage("opscli", `Services unregistered for install dir: ${result.installDir}`);
      writeRoleMessage("opscli", `Components: ${formatComponents(components)}`);
      for (const message of result.messages) writeRoleMessage("opscli", message);
    } catch (error) {
