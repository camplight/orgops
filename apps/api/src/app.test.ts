  ...
  it("accepts agent id in agent routes", async () => {
    ...
    const createAgentRes = await app.request("http://localhost/api/agents", {
      ...
      body: JSON.stringify({
        name: "id-route-agent",
        modelId: "openai:gpt-4o-mini",
        workspacePath: ".orgops-data/workspaces/id-route-agent",
      }),
    });
    expect(createAgentRes.status).toBe(201);

    const listRes = await app.request("http://localhost/api/agents", { headers: { cookie } });
    expect(listRes.status).toBe(200);
    const list = (await listRes.json()) as Array<{ id?: string; name: string }>;
    const created = list.find((row) => row.name === "id-route-agent");
    expect(created?.id).toBeTruthy();
    const agentId = created?.id ?? "";

    const getByIdRes = await app.request(
      `http://localhost/api/agents/${encodeURIComponent(agentId)}`,
      { headers: { cookie } },
    );
    expect(getByIdRes.status).toBe(200);

    const patchByIdRes = await app.request(
      `http://localhost/api/agents/${encodeURIComponent(agentId)}`,
      {
        method: "PATCH",
        headers: { "content-type": "application/json", cookie },
        body: JSON.stringify({ description: "updated by id route" }),
      },
    );
    expect(patchByIdRes.status).toBe(200);

    const getByNameRes = await app.request("http://localhost/api/agents/id-route-agent", {
      headers: { cookie },
    });
    expect(getByNameRes.status).toBe(200);
    const body = (await getByNameRes.json()) as { description?: string };
    expect(body.description).toBe("updated by id route");

    const deleteByIdRes = await app.request(
      `http://localhost/api/agents/${encodeURIComponent(agentId)}`,
      {
        method: "DELETE",
        headers: { cookie },
