...
usage() {
  cat <<'EOF'
Usage:
  install-linux-latest.sh <github-owner/repo> [install-dir]

Example:
  install-linux-latest.sh camplight/orgops /opt/orgops

This script downloads the rolling Linux bundle from:
  https://github.com/<owner>/<repo>/releases/download/bundles-latest/orgops-linux-x64-bundle.tar.gz
...
BUNDLE_URL="https://github.com/${REPO_SLUG}/releases/download/bundles-latest/orgops-linux-x64-bundle.tar.gz"
WORK_DIR="$(mktemp -d)"
...
echo "Downloading latest Linux bundle from ${BUNDLE_URL}"
